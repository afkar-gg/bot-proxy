...
if (!BOT_TOKEN || !CHANNEL) { console.error("[Error] Missing BOT_TOKEN or CHANNEL_ID"); process.exit(1); }
...
<!DOCTYPE html><html><body style="margin:0;padding:0;height:100vh;background:#18181b;color:#eee;display:flex;justify-content:center;align-items:center;font-family:sans-serif;">
  <form method="POST" action="/login-submit" style="flex-direction:column;display:flex;width:260px;">
    <input type="password" name="password" placeholder="Password" required style="padding:10px;margin:6px 0;border:none;border-radius:4px;background:#2a2a33;color:#eee;"/>
...
app.post("/login-submit", express.urlencoded({extended:false}), (req,res) => {
  ...
  if(rec.count >= 10 && Date.now() - rec.last < 5*60*1000) return res.send("⛔ Too many attempts. Try later.");
...
app.get("/dashboard", (req, res) => {
  ...
  const rows = active.map(j=>`
    <tr>
      ...
      <td>${j.timeLeft}m</td>
      <td>${j.status}</td>
      <td><button onclick="location.href='/cancel/${j.username}'" style="background:#ef4444;color:#fff;border:none;border-radius:4px;padding:4px 8px;">Cancel</button></td>
    </tr>`).join("");
  res.send(`
<!DOCTYPE html><html><body style="margin:20px;background:#18181b;color:#eee;font-family:sans-serif;">
  ...
  <div style="max-width:420px;margin:auto;background:#1f1f25;padding:16px;border:1px solid #333;border-radius:8px;">
    <form id="jobForm" style="display:flex;flex-direction:column;">
      ...
      <input name="no_order" placeholder="Order ID" required style="padding:10px;margin:6px 0;background:#2a2a33;color:#eee;border:none;border-radius:4px;"/>
      <input name="nama_store" placeholder="Store Name" required style="padding:10px;margin:6px 0;background:#2a2a33;color:#eee;border:none;border-radius:4px;"/>
      <input name="jam_selesai_joki" type="number" step="any" placeholder="Hours" required style="padding:10px;margin:6px 0;background:#2a2a33;color:#eee;border:none;border-radius:4px;"/>
      <button type="submit" style="padding:12px;background:#3b82f6;color:#fff;border:none;border-radius:4px;">Start Job</button>
    </form>
  </div>
  <h2 style="text-align:center;margin-top:20px;">Active Sessions</h2>
  <table style="margin:auto;width:90%;border-collapse:collapse;color:#eee;">
    <tr style="background:#2a2a33;"><th>User</th><th>Order</th><th>Store</th><th>Left</th><th>Status</th><th>Action</th></tr>
    ${rows}
  </table>
  <script>
...
app.post("/track", (req, res) => {
  ...
  }).then(r=>r.json()).then(data=>{session.messageId=data.id}).catch(console.error);
...
<!DOCTYPE html><html><body style="margin:0;padding:0;height:100vh;background:#18181b;color:#eee;display:flex;justify-content:center;align-items:center;font-family:sans-serif;">
  <div style="text-align:center;">
    ...
    <a href="${uri}" style="color:#3b82f6;">Click if not redirected</a>
  </div>
</body></html>`);
});
...
app.listen(PORT,()=>{
  ...
  const r = m => {
    const t = m.toString();
    if(t.includes("trycloudflare.com")||t.includes("cfargotunnel.com")) console.log("🌐 Tunnel URL:",t.trim());
  };
  tun.stdout.on("data",r);
  tun.stderr.on("data",r);
});